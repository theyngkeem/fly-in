...
class Graph:
    def __init__(self, zones: dict[str, Any], connection: list[dict], nb_drone: int):
    ...
    def creat_zone(self, zones: dict[str, Any]) -> list[Zone]:
        ...
        for zone in zones.keys():
            el = Zone(zone["name"], zone["optional"]["color"],
                      ...
                      True if zone["name"] == ("start_hub" | "end_hub") else False,
                      zone["x"], zone["y"])
            res.append(el)
        return res
    ...
    def creat_conn(self, connection: list[dict]) -> list[Bridge]:
        ...
        for bridge in connection:
            el = Bridge(bridge["first_zone"], bridge["destination"], bridge["optional"]["max_link_capacity"])
            res.append(el)
        return res
